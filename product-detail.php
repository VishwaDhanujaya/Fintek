                ...
                <div class="w-full lg:w-2/5 p-8 lg:p-12 border-b lg:border-b-0 lg:border-r border-gray-100">
                    <div class="sticky top-28">
                        <?php if ($product->isNew()): ?>
                            <span
                                class="inline-block bg-fintek-blue text-white text-xs font-bold px-3 py-1 rounded-full mb-6">NEW
                                PRODUCT</span>
                        <?php endif; ?>
                        ...
                        <h1 class="text-3xl font-bold text-gray-900 mb-4"><?= htmlspecialchars($product->getName()) ?>
                        </h1>
                        <p class="text-gray-600 mb-8 leading-relaxed"><?= htmlspecialchars($product->getShortDesc()) ?>
                        </p>
                    ...
                    <?php if ($product->hasSpecs()): ?>
                        ...
                        <div class="flex overflow-x-auto border-b border-gray-200 mb-8 hide-scrollbar">
                            ...
                            foreach ($product->getSpecGroupNames() as $groupName):
                                ...
                                ?>
                                <button
                                    class="spec-tab-btn whitespace-nowrap py-4 px-6 font-medium text-sm border-b-2 transition-colors <?= $first ? 'border-fintek-blue text-fintek-blue' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' ?>"
                                    data-target="<?= $tabId ?>">
                                        ...
                                        <tbody class="divide-y divide-gray-100">
                                            <?php foreach ($specData as $key => $value): ?>
                                                <tr class="hover:bg-gray-50/50 transition-colors">
                                                    <th scope="row"
                                                        class="px-4 py-4 font-medium text-gray-900 w-1/3 align-top bg-gray-50/30 rounded-l-lg">
